 ...
 * Clean Architecture Domain Shared Kernel - Outbox Message Aggregate Root
 * Represents a robust aggregate root for reliable outbox message handling with status tracking, retries, and auditing.
 * Year: 2025
 */

using AkbarAmd.SharedKernel.Domain.AggregateRoots;
using AkbarAmd.SharedKernel.Domain.Outbox.Events;

namespace AkbarAmd.SharedKernel.Domain.Outbox
{

    /// <summary>
    /// Represents an aggregate root for an Outbox message supporting reliable event/message dispatch.
    /// Encapsulates state transitions, retries, error handling, and audit fields.
    /// </summary>
    public class OutboxMessage : SoftDeletableAggregateRoot<Guid>
    {
        /// <summary>
            ...
            : base(id)
        {
            ...
            // Raise domain event
            AddDomainEvent(new OutboxMessageCreatedEvent(Id, Type, OccurredOn));
        }
        ...
        public void MarkAsProcessed()
        {
            ...
            // Raise domain event
            AddDomainEvent(new OutboxMessageProcessedEvent(Id, ProcessedOn.Value));
        }
        ...
        public void MarkAsFailed(string error)
        {
            ...
            // Raise domain event
            AddDomainEvent(new OutboxMessageFailedEvent(Id, Error, RetryCount));
        }
        ...
        public void MarkAsRetrying()
        {
            ...
            // Raise domain event
            AddDomainEvent(new OutboxMessageRetryingEvent(Id, RetryCount));
        }
        ...
        public void UpdateContent(string newContent)
        {
            ...
            // Raise domain event
            AddDomainEvent(new OutboxMessageContentUpdatedEvent(Id, Content));
        }
    }
}
