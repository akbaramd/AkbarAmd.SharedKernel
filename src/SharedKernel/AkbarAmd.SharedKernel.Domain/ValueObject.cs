 ...
 * ValueObject base class implementing equality, comparison, validation, and event notification.
 * Year: 2025
 */
...
namespace AkbarAmd.SharedKernel.Domain
{
    ...
    public abstract class ValueObject : IEquatable<ValueObject>, IComparable<ValueObject>, IFormattable
    {
        public event EventHandler<ValueChangedEventArgs> ValueChanged;

        protected ValueObject()
        {
            // Validate on construction ensures always valid
            Validate();
        }

        protected abstract IEnumerable<object> GetEqualityComponents();

        protected IReadOnlyList<object> EqualityComponents => 
            GetEqualityComponents().ToList().AsReadOnly();
